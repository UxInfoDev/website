...
const BannerSection = () => {
  ...
  }, [slides.length, rotationSpeed, isPlaying])

  const nextSlide = () => setCurrentSlide((prev) => (prev + 1) % slides.length)
  const prevSlide = () => setCurrentSlide((prev) => (prev - 1 + slides.length) % slides.length)

  const togglePlay = () => setIsPlaying((prev) => !prev) // ✅ NEW
    ...
    <>
      ...
      >
        ...
        {slides.length > 1 && (
          <div
            onClick={togglePlay}
            className="absolute    bottom-4 right-6 z-30 w-10 h-10 bg-white/90 backdrop-blur text-[#0971C8] rounded-full flex items-center justify-center shadow-lg cursor-pointer hover:scale-110 transition"
          >
